...
/* [ConvoyLink patch 1/4] Pins and SPI host come from convoy_pins.h
 * (docs/02 pin map), not from this driver's Kconfig menu - see NOTICE. */
...
/* [ConvoyLink patch 2/4] was `void LoRaInit(void)` reading CONFIG_*_GPIO;
 * now takes its pins from convoy_pins.h and returns esp_err_t instead of
 * assert()ing on SPI failure - docs/01 requires init to fail soft (the app
 * retries every 5 s and shows RADIO?), never abort. See NOTICE. */
esp_err_t LoRaInit(void)
{
	SX126x_SPI_SELECT = CONVOY_PIN_LORA_NSS;
	SX126x_RESET = CONVOY_PIN_LORA_RST;
	SX126x_BUSY	= CONVOY_PIN_LORA_BUSY;
	SX126x_TXEN	= CONVOY_PIN_LORA_TXEN;
	SX126x_RXEN	= CONVOY_PIN_LORA_RXEN;

	ESP_LOGI(TAG, "pins: sck=%d mosi=%d miso=%d nss=%d rst=%d busy=%d txen=%d rxen=%d",
		CONVOY_PIN_LORA_SCK, CONVOY_PIN_LORA_MOSI, CONVOY_PIN_LORA_MISO,
		SX126x_SPI_SELECT, SX126x_RESET, SX126x_BUSY, SX126x_TXEN, SX126x_RXEN);
	...
	spi_bus_config_t spi_bus_config = {
		.sclk_io_num = CONVOY_PIN_LORA_SCK,
		.mosi_io_num = CONVOY_PIN_LORA_MOSI,
		.miso_io_num = CONVOY_PIN_LORA_MISO,
		.quadwp_io_num = -1,
		.quadhd_io_num = -1
	};
	...
	spi_device_interface_config_t devcfg = {
		...
		.mode = 0,
		.spics_io_num = CONVOY_PIN_LORA_NSS,
		.queue_size = 7,
		.flags = 0,
		.pre_cb = NULL
	};
...
bool LoRaSend(uint8_t *pData, int16_t len, uint8_t mode)
{
	...
	if ( txActive == false )
	{
		...
		/* [ConvoyLink patch 4/4] was SetTx(500). A 32-byte SF7/125k/4:5
		 * packet is ~61 ms on air (docs/03), so 200 ms is >3x margin and
		 * matches sx1262.h's documented send timeout - the modem timeout
		 * is what bounds the SYNC wait loop below. See NOTICE. */
		SetTx(200);
