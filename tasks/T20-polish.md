...
- Watchdog coverage: task WDT subscribed on radio/audio/ui tasks with
...
- Defensive sweeps (audit + fix): every queue-full path counts + drops
  oldest; `cl_validate` failures counted per source; NRF24 re-init retry
  (5 s backoff) if init failed at boot — radar UI shows a red `RADIO?`
  status-bar tile meanwhile; GPS silent > 10 s → sats display goes red
...
- [ ] Splash shows correct version; reset-reason banner appears after a
      ...
      comment) 
- [ ] Unplug the NRF24 mid-run: `RADIO?` tile appears, unit keeps
      rendering, replug + `retry` recovers without reboot
